...
- Friday: Alice delivers the real Visuospatial questions + one grid image of 15 visuals to crop into individual stimuli, with level AND expected answer per question. Pipeline: crop -> `ui/public/tasks/` -> task bank rebuilds with image refs. The PATIENT sees printed cards (Lucas, 2026-09-10) - the UI stays clinician-only, showing which card is active plus a small thumbnail so the clinician grabs the right one.
