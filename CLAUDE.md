...
- The PATIENT sees a dedicated display (Alice, 2026-09-11 - supersedes printed cards): `?patient=<session id>` renders PatientScreen, fed by the read-only, sample-free `patient-view` endpoint (phase + stimulus only, never answers or measurements). "Patient screen" link in the topbar opens it. A second window/monitor on the demo laptop needs nothing; a PHYSICAL tablet needs the UI served with `--host`, the API bound to 0.0.0.0 and CORS widened - only do this once the team confirms a tablet.
